...
export default function AnimatedCheckoutStepper({ currentStep }) {
  ...
  return (
    <div className="animated-stepper" style={{ "--progress": `${progress}%` }}>
      <div className="animated-stepper__mobile">
        <span className="animated-stepper__mobile-label" aria-live="polite" aria-atomic="true">
          {mobileLabel}
        </span>
        ...
        {steps.flatMap((step, index) => {
          ...
          const stepNode = isComplete ? (
            <button
              ...
              aria-label={`${step.label} · ${t("results.selected")}`}
            >
